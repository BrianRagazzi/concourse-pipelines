...
cf_properties=$(
  jq -n \
    ...
    --arg ert_blobstore_type "$ERT_BLOBSTORE_TYPE" \
    --arg ert_blobstore_bucket_buildpacks "$ERT_BLOBSTORE_BUCKET_BUILDPACKS" \
    --arg ert_blobstore_bucket_droplets "$ERT_BLOBSTORE_BUCKET_DROPLETS" \
    --arg ert_blobstore_bucket_packages "$ERT_BLOBSTORE_BUCKET_PACKAGES" \
    --arg ert_blobstore_bucket_resources "$ERT_BLOBSTORE_BUCKET_RESOURCES" \
    --arg ert_blobstore_access_key "$ERT_BLOBSTORE_ACCESS_KEY" \
...
#      },
    ...
    if $ert_blobstore_type == "external" then
      {
        ".properties.system_blobstore": { "value": "external" },
        ".properties.system_blobstore.external.buildpacks_bucket": { "value": $ert_blobstore_bucket_buildpacks },
        ".properties.system_blobstore.external.droplets_bucket": { "value": $ert_blobstore_bucket_droplets },
        ".properties.system_blobstore.external.packages_bucket": { "value": $ert_blobstore_bucket_packages },
...
#note changes to errand names in ERT 2.1
ERT_ERRANDS=$(cat <<-EOF
{"errands":[
